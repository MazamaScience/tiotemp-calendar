...
var timeseriesCalendar = function (options) {
    ...
    function drawCal(data) {
        ...
        var getCellInfo = function (d) {
            ...
            if (typeof info !== "undefined") {
                let out;
                if (typeof options.inTooltip !== "undefined") { 
                    out = options.inTooltip(info);
                } else {
                    out = info.mean.toFixed(1) + " " + options.units;
                }
        ...
        var drawTooltip = function (d) {
            ...
            tooltip
                ...
                // NOTE: Look into replacing event based mouse location for whatever react does! 
    ...
    Papa.parse(options.url, {
        ...
        complete: result => {
            ...
            const data = parseWithPapa(result);
...
const Cal = () => {
  ...
  var url = 'https://tools.airfire.org/monitor-data/v4/data?databaseversion=4.0&webserviceapi=4.0&monitors=320311007_01&outputfiletype=csv';
  ...
  React.useEffect(() => {
    ...
    var ts = new timeseriesCalendar({url, el: "poop"})
